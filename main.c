...
void ctx_print_unlocked(ctx_t *ctx) {
  char *msg = ctx->finished ? "" : (ctx->paused ? " ('r' â€“ resume)" : " ('p' â€“ pause)");
  ...
  while (counter < iterations) {
    ...
    for (size_t D = 0; D < 2; ++D) {
      ...
      for (size_t i = 0; i < g_max; ++i) {
        ...
        fe_modp_mul(ss, ss, dx[i]);                           // Î» = (y2 - y1) / (x2 - x1)
        fe_modp_sqr(rx, ss);                                  // Î»Â²
        fe_modp_sub(rx, rx, GStart.x);                        // Î»Â² - x1
        fe_modp_sub(rx, rx, ctx->gpoints[g_idx + i].x);       // rx = Î»Â² - x1 - x2
        fe_modp_sub(dd, GStart.x, rx);                        // x1 - rx
        fe_modp_mul(dd, ss, dd);                              // Î» * (x1 - rx)
        fe_modp_sub(ry, dd, GStart.y);                        // ry = Î» * (x1 - rx) - y1
